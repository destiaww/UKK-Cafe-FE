...
export default function Riwayat() {
  ...
  const [nomer_meja, setnomerMeja] = useState("");
  const [search, setValue] = useState("");
  useEffect(() => {
    const fecthDatas = async () => {
      try {
        const response = await axios.get("http://localhost:9000/transaksi/", {
          headers,
        });
        console.log(response.data.data);
        setTransaksi(response.data.data);

        const res = await axios.get("http://localhost:9000/meja/", { headers });
  ...
  const handleChange = (e) => {
    ...
    const dataFilterred = transaksi.filter((data) => {
      console.log(data.meja)
      return data.meja.nomer_meja === e.target.value;
    });
    console.log(dataFilterred)
  ...
  const handleBayar = async (id) => {
    ...
    try {
      await axios.put(
        "http://localhost:9000/transaksi/" + id,
        updatedStatusTransaksi,
        { headers }
      );
      await axios.put("http://localhost:9000/meja/" + selectedMeja.id, updatedStatusMeja, {headers})
  ...
  //         await axios.delete("http://localhost:9000/transaksi/" + id_transaksi, {headers});
  //         await axios.put("http://localhost:9000/meja/" + selectedMeja.id_meja, updatedStatusMeja, {headers})
  //         window.location.reload()
  //     } catch (error) {
  //         console.error(error);
  //     }
            ...
            {transaksi.map((transaksi) => (
              <tr key={transaksi.id_transaksi} className="hover:bg-gray-50">
                ...
                <td className="px-6 py-4">
                  Meja nomer {transaksi.meja.nomer_meja}
                </td>
